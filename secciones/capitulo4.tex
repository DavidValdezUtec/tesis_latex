...
\begin{enumerate}
	\item Diseño conceptual: definición de arquitectura del sistema,
	      identificación de comunicación entre componentes y
	      especificaciones
	      funcionales.
	...
	\item Implementación: integración de nodos ROS 2, C++, librerías
	      oficiales
	      de los equipos utilizados y protocolos de comunicación entre
	      estos.
...
velocidad \(\boldsymbol{\dot{q}}\) y de valor articular \(\boldsymbol{q}\).
Para
ello, el error de posición \(\boldsymbol{e_p}\) y el de orientación
\(\boldsymbol{e_o}\) (representado en su parte vectorial) se agrupan en un
único
...
\(\boldsymbol{W}\in\mathbb{R}^{6\times 6}\), construida como una matriz
diagonal
por bloques a partir de los vectores escalares \(W_p\) y \(W_o\), ambos
finalmente
...
manera que despejemos la Matriz Hessiana \(\boldsymbol{H}\) y la Matriz
Gradiente
\(\boldsymbol{\nabla}g(\boldsymbol{q})\), en función del Jacobiano y el error
ya
...
El resultado es un valor de paso \(\boldsymbol{\Delta q}\) acotado directamente
a
...
de deslizamiento \(\boldsymbol{S}\) a partir del error de seguimiento
cartesiano
...
operaron efectivamente en un rango de \(100\)--\(120\) Hz
(\(dt\approx8\)--\(9\) ms),
...
\(\boldsymbol{q}_{k+1}=\boldsymbol{q}_k+\boldsymbol{\dot{q}}_{k+1}\,dt\). Para
el
...
\(O(dt^3)\), frente a \(O(dt^2)\) del esquema semi-implícito, por lo que
resulta
...
\begin{figure}[H]
	...
	\caption{Interfaz en PyQt5. Se muestran de izquierda a derecha el área
		de
		configuración, el visualizador 3D de Rviz y el visualizador de
		la
		cámara}
	\label{fig:placeholder}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.8\textwidth}
		\centering
		\includegraphics[width=\linewidth]{images/ensamble_try3
			v11.png}
		\caption*{(a)  Ensamblaje final gripper 2f 85 de la empresa
			Robotiq}
	\end{minipage}
	\hfill
	\begin{minipage}{0.8\textwidth}
		...
		\caption*{(b) Ensamblaje final gripper hand-e de la empresa
			Robotiq}
	\end{minipage}
...
más demandante en aceleración y cambio de dirección. En la segunda etapa, se
validó el
...
\begin{table}[H]
	...
	\begin{tabular}{| c | c | c |}
		...
		Amplitud            & \(A_i\)      & \(0.1\) m           \\
		\hline
		Frecuencia          & \(\omega_n\) & \(2\pi\) rad/s      \\
		\hline
		Constante de tiempo & \(c_0\)      & \(0.05\) s\(^{-1}\) \\
		\hline
	\end{tabular}
	\caption{Constantes de la trayectoria helicoidal}
	\label{tab:constantes_trayectoria_helicoidal}
\end{table}

donde \(x_0\), \(y_0\) y \(z_0\) son las coordenadas del punto inicial de la
trayectoria tomadas
al iniciar la prueba, por lo que no existe un coordenada específica y la prueba
se puede
realizar en cualquier punto del espacio de trabajo del robot dentro de los
límites de alcance del efector final.
...
comandado por el operador en los tres ejes cartesianos. Se medieron los errores
de seguimiento en cada eje cartesiano (\(x\), \(y\), \(z\)) tomando como
consideración
una frecuencia de control de \(250\) Hz, coincidente con la frecuencia de control
del UR5. El esfuerzo de control \(\mu\) en cada articulación se calculó
siguiendo una saturación obtenida de los esfuerzos máximos de torque segun el
fabricante, y se registró el tiempo de finalización de la tarea.
